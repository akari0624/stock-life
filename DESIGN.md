...
| 填空編輯器 | **現在不做**，但內容格式為它而設計；規格見 §6.5，排程見 TODO.md |
...
**唯一的總則：`counter` 與 `flag` 是編譯產物，不進 UI。** 作者畫的是一張圖——事件是框，
結果拉出兩條線（成功／失敗），線上標「馬上」或「幾年後」。編輯器負責把它壓成
`require` / `effects` / `next`。

| 作者想說的 | 編譯成 |
|---|---|
| A 之後接 B | `good.next: { id: 'B' }` |
| A 之後，過幾年才 B | `next: { id, afterYears }` |
| 成功接 B、失敗接 C | good / bad 各一份 `next` |
| B 和 C 最後都回到 D | 兩邊都指 D（匯流天然成立，不需要特別支援） |
| 這一格一輩子只演一次 | `once: true` |
| 三件事都發生過才開下一段 | 三格各 `+1` 到同一個計數器，D 要求 `>= 3`（三格都要 `once`） |
| 走了這條就永遠看不到那條 | 分岔時把段號推到不同區段（good `+1`、bad `+11`） |

前五列是 §7.2 的宣告欄位，後兩列還是靠計數器——它們是**編輯器該藏起來的部分**，
不是作者該學的東西。
...
|  | 入口事件 | 劇情段落 |
|---|---|---|
| `weight` | > 0，要跟其他八十幾個事件搶那一年唯一的抽籤位 | `0`，永遠不進隨機池 |
| `require` | 決定它什麼時候有資格被抽 | 被 `next` 指到時**不檢查**（§7.2） |
| 怎麼進來 | 抽籤 | 只走箭頭 |

⚠️ **鏈接是精確的，入場是機率的。** 這兩件事在作者腦中會混在一起，然後他會抱怨
「我的故事沒出現」。實測：四段故事、第一段用常見權重 8，**29% 的人生從頭到尾沒遇到
第一段，但只要遇到了，96% 會把四段講完**，段與段固定隔一年。

所以入口框要顯示「約 X% 的人生會遇到、平均 N 歲」（跑 200 局算出來），段落框只顯示
誰指向它。作者才會知道：箭頭之後的事我保證，箭頭之前的事我不保證。
...
- **id 防撞。** `mergeContentPacks` 是純 `flatMap`，沒有任何重複 id 檢查。兩個包都叫
  `first_love`：抽籤池裡有兩份，權重相加（實測 10 + 8 = 18）。更糟的是那兩份的分工——
  `weightedPick` 抽到哪一份就演**那一份**的 prompt 與選項文案，但玩家按下去之後
  `resolve()` 走 `byId.get()`，**效果與 `next` 一律用最後載入的那份**。可能讀到 A 的句子、
  吃到 B 的效果，而且不報錯。**UGC 開放前必須修**（規格與選項見 TODO.md #10）。
...
- **存檔指紋懸崖。** 指紋是 `pack id + version` 的雜湊，對不上就在重播任何一個 command
  之前直接拒絕載入（§5.1）。社群包會一直改版，等於作者每更新一次玩家就掉存檔。要先
  決定策略——例如只有 breaking change 才進指紋，或存檔記錄完整包內容。
- **文字審核。** 陌生人的字會直接印在畫面上。`checkTrust` 目前只擋大小與數量。

#### 6.5.5 還沒定案的一條分岔

**選項的形狀。** 現在是 `safe` / `normal` / `bold` 三檔鎖死（schema `.length(3)`）、
百分比直接印在按鈕上、效果 good/bad 共用。這是為「人生模擬」設計的形狀。

想寫故事的人第一天就會撞到：我要兩個選項、我不要顯示機率、我要一段沒有骰子的純劇情、
我要這個選項只有認知夠高才出現。

**先決定網友是來寫哪一種，再決定 schema。** 它會決定編輯器，編輯器會決定所有人已經
寫好的內容——這是往後最難改的一件事，所以這裡刻意不預設答案。

#### 6.5.6 第一步不是打開表單，是問 AI

**現在寫內容的人，第一個動作是叫 AI 產一批。** 編輯器如果只做「填完表單匯出 JSON」，
它就站在流程的最後一格——而那一格沒人站。所以要補的是**第一格與第二格**：
`產提示詞 → 貼回 JSON → 在表單裡改`。表單不是產內容的地方，是**改**內容的地方。

住在 `editor/aiPrompt.ts`（產提示詞）、`editor/paste.ts`（吃回貼的 JSON）、
`components/AiImport.tsx`（那一個對話框）。三個決定：

1. **提示詞是算出來的，不是寫死的一段字。** 條件欄位、運算子、效果型別、統計欄位名、
   場景素材 id、三檔選項的形狀、基準成功率——全部來自跟編輯器自己同一份來源
   （`listFacadeFields()`、`STAT_KEYS`、`StateEffect['type']`、`assetCatalogue()`、
   `BASE_SUCCESS_CHANCE`）。理由跟 §6.5.3 #2 一模一樣：手抄的第二份清單一定會漂掉，
   而提示詞漂掉的代價是 AI 產出一批**看起來很像、但載不進去**的事件。
   ⚠️ 提示詞裡的範例事件直接取**官方包裡真的在跑的那一則**（載入 `core-tw` 後從
   `baseline.sampleEvents` 拿），不手抄一份——手抄的範例會變成過期的規格，
   而 AI 抄範例比讀規則認真得多。
2. **貼上要寬容。** AI 會回一整包、會只回 `{ "events": [...] }`、會回一個裸陣列、
   會包一層 markdown 圍籬、會在前面加一句「以下是您要的內容：」、會留一個結尾逗號。
   這幾種一律吃得下（認不出來才報錯），而且**不在匯入時驗證**——跟匯入檔案同一個決定：
   AI 產的事件本來就會有幾格是壞的，擋在門外的話作者連看都看不到。壞掉的格子由
   §6.5.3 #3 的即時驗證標紅，作者在表單裡修。**貼進來壞掉是正常的，修不動才是問題。**
3. **預設是附加，不是取代。** 撞到 id 時改掉**新來的那一批**，並同步改掉那一批內部
   指向它的箭頭——AI 產的一串連續事件不可以因為改名就斷成散裝。（跟 §6.5.4 第一條的
   跨包 id 防撞是兩件事：那個要引擎修，這個是同一份草稿裡的事，編輯器自己就該處理。）
