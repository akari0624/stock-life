...
**本步驟不做**：大量內容（S18 才做）、編輯器 UI（`TODO.md` #1，不做）、
從 URL 或檔案載入（`TODO.md` #2，S17 才做）。
...
**目標**：時代這張舞台。先做 `random`（`tw-history` 留到 S18）。
...
**本步驟不做**：`tw-history`（S18）、真實公司名稱、任何機會判定。
...
嚴格遵守 S7 的「本步驟不做」清單——tw-history 留到 S18。
```
</details>
...
**本步驟不做**：實際調平衡數值（內容不足，S18 再調）、任何 UI。
...
**目標**：樣式基礎設施。**只有 alias 層進 `@theme`** 是本步驟的核心紀律。

**依據**：`DESIGN.md` §10.3、§10.5；`TODO.md` #9

**前置**：S1（不需等引擎）

**產出**：
- Tailwind v4 設定
- `apps/web/src/styles/tokens.global.css`：Tier 1 原色，純 `:root`，**不進 `@theme`**
- `apps/web/src/styles/tokens.alias.css`：Tier 2 語意，**進 `@theme`**
- 至少兩個主題（`default`、`scoreboard`），切換只覆寫 alias
- Typographic：semantic type roles（`display`/`title`/`body`/`caption`/`numeric`），
  body 行高 1.7–1.8，`numeric` 含 `tabular-nums`（§10.5）
- 字體決策：先採系統字體 stack（`TODO.md` #9 建議），並在 `TODO.md` 記錄結論

**完成判準**：
- `bg-surface-raised`、`text-loss` 這類 alias utility 可用
- `bg-g-green-500` 這類原色 utility **不存在**（有測試或明確驗證步驟）
- 切 `data-theme` 整頁換色，且 Tier 1 未被修改
- 一個示範頁展示所有 type role 與主題切換

**本步驟不做**：真正的遊戲畫面（S15）、director（S13）。

<details><summary>起始指令</summary>

```
讀 DESIGN.md 的 §10.3、§10.5，TODO.md 的 #9，以及 PLAN.md 的 S12。
執行 S12：設定 Tailwind v4 與三層 design token。核心紀律是「只有 alias 層
進 @theme，global 原色留在 :root」——請驗證 bg-g-green-500 這類原色 utility
確實不存在。另外實作 semantic type roles（注意 §10.5 的三個繁中排版坑）。
嚴格遵守 S12 的「本步驟不做」清單。
```
</details>
...
- `apps/web/src/presentation/assets/AssetResolver.ts`：
  id → 素材，**所有視覺/聽覺資源只透過 id 引用，不得硬編碼路徑**
- 每種型別都有 fallback：角色→名字色塊、背景→漸層、SFX→靜音、FX→CSS 動畫
- `apps/web/src/presentation/audio/AudioBus.ts`：存在但 no-op
...
no-op 的 AudioBus、以及 presentation/stage 的 Tier 3 component token。
驗收重點：引用不存在的素材 id 要 fallback 而非崩潰；塞一張真圖進 manifest
就能生效且 domain 零改動。完成後把 TODO.md #5 的邊界清單打勾。
```
</details>
...
**本步驟不做**：存檔（S16）、內容包匯入（S17）、大量內容（S18）。

<details><summary>起始指令</summary>

```
讀 DESIGN.md 的 §4、§10.1，PLAN.md 的 S15，並參考 yakyulife 的版面
（深色、文字流、年度摺疊）。
執行 S15：組裝畫面狀態機與三個主要畫面，UI 用 useSyncExternalStore 訂閱 sim。
...
```
讀 DESIGN.md 的 §5.1，TODO.md 的 #4，以及 PLAN.md 的 S16。
執行 S16：實作存檔（存 seed + fingerprint + commandLog，不存狀態快照）、
...
```
讀 DESIGN.md 的 §6.4，TODO.md 的 #2，以及 PLAN.md 的 S17。
執行 S17：實作 FileSource / PasteSource（新增來源不得改呼叫端）、
...
```
讀 DESIGN.md 的 §7.4、§2、§1，TODO.md 的 #8，以及 PLAN.md 的 S18。
執行 S18：實作 TwHistoryWorldGenerator（標的一律暗示但不指名）、
...
**S12 可以與 S2–S11 並行**（只依賴 S1），如果你想同時開兩條線。
