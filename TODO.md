...
還沒做的是 §6.5.4 那幾件「編輯器救不了、引擎要先補」的事（其中 id 防撞已經展開成
本文件 **#10**），以及 §6.5.5 那條選項形狀的分岔——那兩節才是接下來的工作清單。
...
**AI 產事件**（產提示詞 → 貼回 JSON → 在表單裡改，見 DESIGN.md §6.5.6）。
**還沒做**：範例模板庫——不過「填空」的入口現在是 AI 那一條，範例模板的優先度因此降了。
...
## 10. 內容包覆蓋官方內容（同 id 取代）

**狀態**：**未做，且 UGC 開放前必須做**（`DESIGN.md` §6.5.4 第一條）。
不是「延後的功能」，是一個已經會出事的行為——`apps/web` 今天就收得下第三方包。

**目標**：作者能明確地說「我要換掉官方的 `overtime_crunch`」，而且遊戲照做、
玩家看得到自己換掉了什麼。

**現在的行為（實測，非推論）**：拿 core-tw 的 `overtime_crunch`（weight 10）
改寫成同 id 的包裝進去 ——
...
合併後這個 id 有幾份： 2
抽籤池裡的權重總和： 18 （原本 10）
byId 認的是哪一份（決定效果與 next）： 【我改寫的版本】
validateMergedContent 有抱怨嗎： []
```

也就是說：**沒有「取代」這回事，只有「兩份同時存在」**。而且兩份的分工是壞的——
`weightedPick` 從攤平的陣列抽，抽到哪一份就演那一份的 `prompt` 與選項文案；
但玩家按下去之後 `resolve()` 走的是 `byId.get()`（`EventSystem.ts:142`），
**效果與 `next` 一律用最後載入的那份**。可能讀到官方的句子、吃到 mod 的效果，
而且全程不報錯。

⚠️ `apps/cms` 的「用遊戲的載入器驗收」也抓不到：它跑的就是同一套 merge。
編輯器只擋得住同一個包內部的重複 id。

**順帶做完的 manifest 稽核**（哪些欄位今天真的有作用）：

| 欄位 | 現在做什麼 |
|---|---|
| `id` + `version` | 指紋（`computeFingerprint`，排序後雜湊 `id@version`）→ 分享碼與存檔；`PackLibrary` 用 id 判斷「升級同一個包」 |
| `engineApi` + `facadeVersion` | `checkCompatibility()`，**唯一擋得下載入的欄位** |
| `assets` | id → 檔案；`mergeAssetManifests()` 後載入覆蓋先載入，沒對到就走 §6.3 fallback |
| `provides` | **純顯示**。`PacksScreen` 印出來而已，`checkTrust()` 用的是實際陣列長度；實測宣告 999 個事件、實際 0 個，照收 |
| `provides.worldGenerators` | **全 repo 沒有任何一行讀它**（世界產生器是引擎註冊的，內容包沒有提供產生器的管道） |
| `requires` | **完全沒有實作**。實測指向一個不存在的包，照收；真的少載入時是靠 `validateMergedContent()` 的斷鏈訊息間接發現 |

**兩條路，要先拍板**：

- **A · 後載入覆蓋先載入**：`mergeContentPacks` 同 id 取代而非併存，並回報覆蓋了哪些 id。
  改動最小，也符合「我裝了一個改寫包」的直覺；代價是**沉默的覆蓋**——兩個 mod 撞名時
  誰贏只取決於安裝順序。
- **B · manifest 明示 `overrides: ["overtime_crunch"]`**：沒宣告的重複 id 直接視為錯誤退回。
  作者不會不小心蓋到東西，而且這會順帶逼 `requires` 長出牙齒——一個宣稱要覆蓋 core-tw
  事件的包，本來就該先確認 core-tw 在場且版本對得上。
  代價是多一個作者要學的欄位，`apps/cms` 要跟著出 UI。

建議 **B**，但 A 的合併邏輯是 B 的前半段，先做 A 不會白做。

**必須先做到的邊界**：

- [x] 合併是一個具名的地方（`mergeContentPacks`），不是散在各系統裡
- [x] 跨包的完整性檢查有地方去（`validateMergedContent`），且它看的是**玩家實際載入的組合**
- [ ] `mergeContentPacks` 回報覆蓋清單（誰蓋了誰），而不是靜靜地合併
- [ ] 覆蓋的結果要能顯示在 `PacksScreen`：玩家要看得到「這個包改寫了官方的 3 個事件」
- [ ] `requires` 真的檢查（id 在場 + 版本相容），否則 B 案的宣告沒有意義
- [ ] `apps/cms` 的「驗收」要跟著抓得到跨包撞名——現在它跟遊戲一樣看不見

**風險提醒**：覆蓋會改變抽籤池，但指紋只雜湊 `id@version`（§5.1）。
所以**覆蓋規則本身一改，所有既有存檔的重播結果就會變**，而指紋不會察覺。
動這裡的同時要一併決定：是把覆蓋關係算進指紋，還是升 `SAVE_SCHEMA_VERSION` 讓舊存檔明確失效。
